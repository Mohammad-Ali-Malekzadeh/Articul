...
دارد support random access مشخص میشه که میشه از یه ایندکس دیگه شروع به خواندن کرد یا به عبارتی قابلیت  .seekable() که البته جلوتر با کمک 


@ methods of StringIO:
1. StringIO.getvalue(): محتوای داخیل فایل رو به صورت استرینگ بر میگردونه
...
------f = open("demofile.txt", "r") برمیگردونه False در این مثال
---StringIO.readable()
---StringIO.writable() 
---StringIO.seekable():
